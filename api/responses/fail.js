...
module.exports = function fail(optionalData) {
  ...
  var result = {
    status: 'FAIL',
    message: optionalData || "Operation Fail"
  };
  return res.status(statusCodeToSet).send(result);

  // If no data was provided, use res.sendStatus().
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.fail()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.fail()` called with an Error:', optionalData);
