...
module.exports = function error(optionalData) {
  ...
  var result = {
    status: 'ERROR',
    message: optionalData || "Internal Server Error",
  };
  return res.status(statusCodeToSet).send(result);
  // If no data was provided, use res.sendStatus().
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.error()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.error()` called with an Error:', optionalData);
